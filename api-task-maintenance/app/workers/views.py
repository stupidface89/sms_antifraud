...
class GetTask(views.APIView):
    ...
    def get_task(self):
        ...
        count_operations_q = Q(operation__time_created__year=now.year,
        ...
        get_task = (
            ...
            .annotate(
                all_operations=Count('operation', filter=count_operations_q),
                started=Count('operation', filter=count_operations_q & Q(operation__status='started')),
                ratio_all=ExpressionWrapper(
                    F('all_operations') * 1.0 / (F('count') / 24), output_field=FloatField()
                ),
                ratio_started=ExpressionWrapper(
                    F('started') * 1.0 / (F('count') / 24), output_field=FloatField()
                ),
            )
        )
